...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  return (
    <main className="pt-16 p-4 container mx-auto">
      <h1 className="text-2xl font-mono font-bold text-amber-400">{message}</h1>
      <p className="text-neutral-400 mt-2">{details}</p>
      {stack && (
        <pre className="w-full p-4 overflow-x-auto mt-4 bg-[#111] border border-[#2a2a2a] rounded font-mono text-sm text-neutral-400">
          <code>{stack}</code>
        </pre>
      )}
    </main>
